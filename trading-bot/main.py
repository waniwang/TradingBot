...
# ---------------------------------------------------------------------------
# EP News EOD scan + execute (mirrors EP Earnings flow)
# ---------------------------------------------------------------------------


def job_eod_ep_news_scan(
    config: dict,
    client: AlpacaClient,
    db_engine,
    notify=None,
    ...
    """
    3:00 PM ET -- EOD scan for EP news gap-up candidates (non-earnings).

    Phase 1: Spikeet universe filters (scanner) excluding earnings
    Phase 2: Strategy A + B evaluation (news-specific entry filters)
    Phase 3: Persist qualifying entries for 3:50 PM execution
    """
    global _watchlist, _ep_news_entries
    if not is_trading_day(client):
        logger.info("EOD EP news scan skipped -- not a trading day")
        return
    _set_phase("eod_ep_news_scan")
    ...
    if "ep_news" not in enabled:
        logger.info("EP News strategy disabled -- skipping EOD EP news scan")
        _set_phase("idle")
        _set_progress()
        return

    try:
        # Phase 1: Scanner (universe filters, exclude earnings)
        _set_progress("Scanning EP news candidates")
        ...
        # Phase 2: Fetch daily bars for strategy evaluation
        _set_progress("Fetching daily bars for news strategy evaluation")
        tickers = [c["ticker"] for c in candidates]
        daily_bars = client.get_daily_bars_batch(tickers, days=300)
        logger.info("Fetched daily bars for %d/%d tickers", len(daily_bars), len(tickers))

        # Phase 3: Evaluate Strategy A + B filters
        _set_progress("Evaluating News Strategy A + B filters")
        ...
        if entries:
            ...
            # Store strategy entries for 3:50 PM execution job
            _ep_news_entries = entries

            # Persist each strategy entry with metadata
            for entry in entries:
                _persist_ep_news_entry(entry, today, db_engine)
...
def _persist_ep_news_entry(entry: dict, scan_date, db_engine):
    """Persist an EP news strategy entry to the watchlist with strategy metadata."""
    from db.models import Watchlist
    import json as _json
...
def job_ep_news_execute(
    ...
    """
    3:50 PM ET -- Execute entries for EP news strategies.

    Reads strategy-tagged entries prepared by the 3:00 PM scan,
    runs risk checks, and places limit orders with GTC stops.
    Strategy A uses -7% stop, Strategy B uses -10% stop.
    If a stock passes both A and B, enters ONE position using the tighter stop (A's -7%).
    """
    global _ep_news_entries
    from signals.base import SignalResult

    if not is_trading_day(client):
        return

    entries = _ep_news_entries
    if not entries:
        logger.info("EP news execute: no entries to execute")
        return

    _set_phase("ep_news_execute")
