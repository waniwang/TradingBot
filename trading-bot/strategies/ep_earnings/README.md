...
3. **3:50–3:59 PM** — `job_execute` queries `Watchlist.stage="ready"` for `setup_type="ep_earnings"` and places orders. Fires every minute (10 attempts) so a briefly-down bot or a transient Alpaca error still gets a trade in before close. **DB-driven + idempotent** — nothing held in memory between scan/confirm and execute, and replays are blocked by the open-Position guard and the <10-minute non-terminal Order guard, so process restarts and multi-fire retries are both safe.
